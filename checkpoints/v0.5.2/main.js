...
const CONFIG = {
  ...
  sim: {
    ...
    seasonPeriod: 480,     // seconds per full spring→summer→autumn→winter cycle (at 1× speed)
  },
};

// Energy / metabolism. Three-part cost model:
//   1) MAINTENANCE_COST per tick after the grace period (basic respiration)
//   2) AGE_COST per tick × current age (senescence — even leafy plants die
//      eventually, so the population always turns over)
//   3) energy is capped at MAX_ENERGY so a fat parent can't bank infinite
//      reserves and dominate the gene pool forever.
// Tuned so a leafless stump dies in ~50 ticks past grace (~25 s @ 2 Hz)
// and a leafy plant in full sun lives ~150–250 ticks before old age catches
// up. v0.4.13: tied capture to leaf depth in light.wgsl so leafless
// segments contribute almost nothing to gain.
const ENERGY_CONFIG = {
  SEED_ENERGY:       100,    // starting energy of every fresh seed
  MAX_ENERGY:        140,    // ceiling — no infinite banking
  GRACE_TICKS:         8,    // no metabolic cost until plant is this old
  LIGHT_PER_ENERGY: 4000,    // measuredLight units required to make 1 energy unit
  MAINTENANCE_COST:  1.0,    // energy spent per tick once past grace period
  AGE_COST:        0.012,    // additional cost per tick × current age
  DECAY_TICKS:        24,    // sim-ticks from death (energy 0) to slot replacement
                             //   ~12 s @ 2 Hz: tilt over 0–70 %, shrink + fade 70–100 %
};

// Pure energy step. Returns the new (capped) energy value. Mirrored in
// /tests/energy.test.mjs — keep the two in sync.
...
// Seasonal cycle: t / seasonPeriod, wrapped to [0, 1). The leaf shader
// uses this to fade leaves through spring → summer → autumn → winter.
// 0.00–0.25 spring, 0.25–0.50 summer, 0.50–0.75 autumn, 0.75–1.00 winter.
function computeSeason(t, period) {
  const s = (t / period) - Math.floor(t / period);
  return s < 0 ? s + 1 : s;
}

function seasonName(s) {
  if (s < 0.25) return 'spring';
  if (s < 0.50) return 'summer';
  if (s < 0.75) return 'autumn';
  return 'winter';
}

// Dynamic wind gust intensity. Sum of two slow sines so the field
// experiences periods of calm punctuated by gentle gusts. Returns a
// multiplier in roughly [0.25, 1.75] — 1.0 is the average.
function computeWindGust(t) {
  const slow = Math.sin(t * 0.18);              // ~35 s period
  const slower = Math.sin(t * 0.07 + 1.7);      // ~90 s period
  return 1.0 + slow * 0.45 + slower * 0.30;
}
...
function createFrameUniforms(device) {
  ...
  return {
    ...
    update(p) {
      ...
      cpu[80] = p.width; cpu[81] = p.height; cpu[82] = p.season != null ? p.season : 0;
      cpu[83] = p.windGust != null ? p.windGust : 1.0;
    },
    upload(queue) {
...
function createSimResources(device, maxPlants, segsPerPlant) {
  ...
  // Energy ledger. Each plant starts with SEED_ENERGY units. Every evolve tick:
  //   gain = measuredLight[p] / LIGHT_PER_ENERGY  (canopy sun → photosynthesis)
  //   cost = MAINTENANCE_COST * size_factor       (respiration / structural cost)
  // When energy hits 0, the plant has starved and is replaced with a mutated
  // child of a successful (= high-energy) parent. Plants with no leaves capture
  // no canopy light, so they starve quickly. Plants with healthy canopies in
  // sun stay alive and reproduce. This is the natural-selection signal.
  const cpuPlantEnergy = new Float32Array(maxPlants);
  // Per-plant decay progress: 0 = alive, (0, 1] = dying (tilt + decay), >1 → slot
  // is replaced next evolve tick. Packed into genome._pad0 each tick so the
  // vertex shaders can read it without a new bind group.
  const cpuPlantDeathProgress = new Float32Array(maxPlants);
  for (let p = 0; p < maxPlants; p++) {
    ...
    cpuPlantEnergy[p] = ENERGY_CONFIG.SEED_ENERGY;
    cpuPlantDeathProgress[p] = 0;
  }
  ...
  return {
    ...
    cpuGenomes, cpuPlantPositions, cpuPlantBirthTick, cpuPlantEnergy, cpuPlantDeathProgress,
...
function typeBounds(t) {
  ...
  // tree — max ~5.7 m. Most trees come out 2–4 m, which reads as a
  // realistic-sized tree on a 16 m² field.
  return {
...
function writeGenome(gf, plantIdx, g, decayProgress) {
  ...
  gf[off + 10] = decayProgress != null ? decayProgress : 0;   // _pad0: death/decay 0..1
  gf[off + 11] = 0;
}
...
function buildDebugText() {
  ...
  if (s.season != null) lines.push(`season: ${seasonName(s.season)} (${(s.season * 100).toFixed(0)} % of year)`);
  if (s.births != null) lines.push(`births: ${s.births}  (mutated offspring planted so far)`);
  if (s.lastDeaths != null) lines.push(`deaths(last tick): ${s.lastDeaths}  (plants that ran out of energy)`);
  if (s.avgEnergy != null) lines.push(`avgEnergy: ${s.avgEnergy.toFixed(1)} / ${ENERGY_CONFIG.SEED_ENERGY}  (mean across population)`);
...
async function main() {
  ...
  try {
    ...
    function showInspect(idx) {
      ...
      const energy = sim.cpuPlantEnergy[idx];
      const decay = sim.cpuPlantDeathProgress[idx];
      ...
      const stateLabel = decay > 0 ? `dying (${(decay * 100).toFixed(0)} %)` : 'alive';
      ...
      insBody.innerHTML = [
        ...
        ['age',         `${age} ticks`],
        ['light',       `${light}`],
        ['energy',      `${energy.toFixed(1)} / ${ENERGY_CONFIG.SEED_ENERGY}`],
        '<hr/>',
    ...
    const EV = {
      ...
      births: 0,
      lastDeaths: 0,        // # starved plants in the most recent tick
    ...
    function replacePlant(targetIdx, parentIdx, tick) {
      ...
      sim.cpuPlantEnergy[targetIdx] = ENERGY_CONFIG.SEED_ENERGY;
      sim.cpuPlantDeathProgress[targetIdx] = 0;
      ...
      // 1. Update this plant's genome slot (decay progress reset to 0).
      writeGenome(EV.genomeBuf, 0, newGenome, 0);
    ...
    function evolveStep(tick) {
      ...
      // 1) Energy + decay ledger. Each plant is in one of three states:
      //    - alive (deathProgress == 0, energy > 0): photosynthesise as usual.
      //    - alive but just starved → enter dying state (deathProgress = epsilon).
      //    - dying (deathProgress > 0): tilt over and fade, no more energy
      //      changes. After DECAY_TICKS ticks, the slot is reused for a
      //      mutated child of a high-energy parent.
      const fullyDecayed = [];        // slots ready to replace
      const justStarved = [];          // count for HUD only
      const scores = new Array(CONFIG.sim.maxPlants).fill(0);
      let aliveCount = 0;
      for (let i = 0; i < CONFIG.sim.maxPlants; i++) {
        const age = tick - sim.cpuPlantBirthTick[i];

        if (sim.cpuPlantDeathProgress[i] > 0) {
          // Already dying — advance the decay clock and check completion.
          sim.cpuPlantDeathProgress[i] += 1 / ENERGY_CONFIG.DECAY_TICKS;
          if (sim.cpuPlantDeathProgress[i] >= 1.0) {
            fullyDecayed.push(i);
          }
          continue;   // dying plants don't gain/lose energy and aren't parents
        }

        sim.cpuPlantEnergy[i] = tickPlantEnergy(
          sim.cpuPlantEnergy[i],
          sim.measuredLight[i],
          age,
          ENERGY_CONFIG,
        );

        if (age >= ENERGY_CONFIG.GRACE_TICKS && sim.cpuPlantEnergy[i] <= 0) {
          // Just starved this tick: kick off the dying animation. Use a
          // small positive value so the next tick advances it normally.
          sim.cpuPlantDeathProgress[i] = 1 / ENERGY_CONFIG.DECAY_TICKS;
          justStarved.push(i);
        } else if (age >= ENERGY_CONFIG.GRACE_TICKS) {
          scores[i] = Math.max(1, sim.cpuPlantEnergy[i]);
          aliveCount++;
        }
      }

      // Need at least one viable parent to repopulate.
      if (aliveCount < 2) {
        // Still upload genomes (decay progress changes need to reach GPU).
        uploadGenomeBuffer();
        EV.lastDeaths = justStarved.length;
        return;
      }

      // 2) Replace fully-decayed slots with mutated children of high-energy
      //    parents. Cap so a population crash doesn't stall the frame.
      const REPLACE_CAP = 32;
      const n = Math.min(fullyDecayed.length, REPLACE_CAP);
      for (let r = 0; r < n; r++) {
        const targetIdx = fullyDecayed[r];
      ...
      // 3) Push the per-plant decay progress to the GPU so vertex shaders
      //    can tilt + shrink dying trees. Cheap: 256 plants × 48 B = 12 KB.
      uploadGenomeBuffer();
    }

    // Re-pack every plant's genome (with current decay progress) into the
    // GPU buffer in one writeBuffer call.
    const allGenomesBuf = new Float32Array(CONFIG.sim.maxPlants * 12);
    function uploadGenomeBuffer() {
      for (let i = 0; i < CONFIG.sim.maxPlants; i++) {
        writeGenome(allGenomesBuf, i, sim.cpuGenomes[i], sim.cpuPlantDeathProgress[i]);
      }
      ctx.device.queue.writeBuffer(sim.genomeBuffer, 0, allGenomesBuf);
    }
    ...
    function loop(now) {
      ...
      const season = computeSeason(simTime, CONFIG.sim.seasonPeriod);
      const windGust = computeWindGust(simTime);
      debugState.season = season;
      debugState.windGust = windGust;
      if (seasonEl) seasonEl.textContent = `${seasonName(season)} ${(season * 100).toFixed(0)}%`;
      frame.update({
        view: camera.view,
        proj: camera.proj,
      ...
      if (elapsed > CONFIG.sim.startDelay && timeScale > 0) {
        ...
        if (simAccum >= tickPeriod) {
          ...
          evolveStep(simTick);
          // Population stats for the HUD.
          debugState.lastDeaths = EV.lastDeaths;
          let energySum = 0;
          for (let p = 0; p < CONFIG.sim.maxPlants; p++) energySum += sim.cpuPlantEnergy[p];
          debugState.avgEnergy = energySum / CONFIG.sim.maxPlants;
        }
      }
      ...
      // Async per-plant light readback every ~250 ms — feeds the energy
      // ledger in evolveStep so fading plants get killed off promptly when
      // they get shaded out instead of lingering on stale sun data.
      if (now - lastLightReadback > 250) {
