...
function createSimResources(device, maxPlants, segsPerPlant) {
  ...
  // so both growth.wgsl and leaf.wgsl can index by plantIdx.
  const cpuGenomes = [];
  for (let p = 0; p < maxPlants; p++) {
...
function makeGenome(seedBase, plantIdx) {
  ...
  return {
    branchAngle: 0.40 + rand() * 0.40,   // 0.40 .. 0.80 rad
    branchProb:  0.55 + rand() * 0.35,   // 0.55 .. 0.90
    lenScale:    0.80 + rand() * 0.10,
    radScale:    0.72 + rand() * 0.10,
    maxDepth:    5 + Math.floor(rand() * 3),  // 5..7
    growthBias:  0.40 + rand() * 1.00,
