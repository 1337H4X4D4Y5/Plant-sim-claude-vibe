...
// 3×3 max-pooled canopy *height* read off the grid. Taller canopies =
// darker shadow. Cells where nothing landed read 0.
fn canopyShadow(worldPos: vec3<f32>) -> f32 {
  ...
  let canopyMeters = f32(maxCm) / HEIGHT_SCALE;
  // 0 m → no shadow. ~6 m → ~0.30 darkening. Clamped to never go below 0.25.
  return clamp(1.0 - canopyMeters * 0.10, 0.25, 1.0);
}
