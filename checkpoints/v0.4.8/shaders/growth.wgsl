...
fn grow(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // Defensive height cap. Worst-case math allows ~14 m trees with current
  // typeBounds; anything above 20 m is a sign that NaN/Inf or some other
  // glitch leaked in. Terminate the tip and let it stop growing.
  if (tipPos.y > 20.0 || tipPos.y != tipPos.y) {
    segments[i].flags = s.flags & ~TIP_BIT;
    return;
  }
