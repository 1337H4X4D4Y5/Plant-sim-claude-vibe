...
fn grow(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  if (s.len <= 0.0 || s.len > 4.0 || s.len != s.len) {
    segments[i].flags = s.flags & ~TIP_BIT;
    return;
  }
  if (s.pos.y != s.pos.y || abs(s.pos.y) > 20.0) {
    segments[i].flags = s.flags & ~TIP_BIT;
    return;
  }
  ...
  // Defensive height cap. Worst-case math with v0.4.9 typeBounds allows
  // ~8.5 m trees; 12 m is comfortably above legitimate and below "stick"
  // territory. Terminate cleanly so render never sees runaway segments.
  if (tipPos.y > 12.0 || tipPos.y != tipPos.y) {
    segments[i].flags = s.flags & ~TIP_BIT;
    return;
  }
