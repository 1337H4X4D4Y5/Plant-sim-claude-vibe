...
// Energy / metabolism. Tuned so a leafy plant in full sun comfortably gains
// energy each tick, while a leafless stump or a heavily-shaded plant loses
// ~0.6 energy/tick and dies in ~150 ticks (≈ 75 s of sim @ 2 Hz). Tweak
// these to make the ecosystem more or less Darwinian.
const ENERGY_CONFIG = {
  SEED_ENERGY:       100,    // starting energy of every fresh seed
  GRACE_TICKS:        10,    // no metabolic cost until plant is this old (lets it grow leaves)
  LIGHT_PER_ENERGY: 4000,    // measuredLight units required to make 1 energy unit
  MAINTENANCE_COST:  0.6,    // energy spent per tick once past grace period
  SIZE_COST:         0.0,    // (currently unused) extra cost per segment
};

// Pure energy step. Returns the new energy value. Mirrored in
