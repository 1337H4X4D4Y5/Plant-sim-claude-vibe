...
function createGrowthPipeline(device, code) {
  const module = device.createShaderModule({ code, label: 'growth.wgsl' });
...
function createBranchRenderer(ctx, frame, shaderCode, segmentsBuffer, maxSegs) {
...
function buildDebugText() {
  ...
  lines.push(`segs: ${s.segs == null ? '--' : s.segs}`);
...
async function main() {
  ...
  try {
    ...
    const growth = createGrowthPipeline(ctx.device, growthCode);
    const growthBindGroup = ctx.device.createBindGroup({
    ...
    const branches = createBranchRenderer(ctx, frame, branchCode, sim.segments, CONFIG.sim.maxSegs);
    ...
    async function queueCounterReadback() {
      ...
      try {
        ...
        const v = new Uint32Array(stagingBuffer.getMappedRange())[0];
        stagingBuffer.unmap();
        segsEl.textContent = String(v);
        debugState.segs = v;
      } catch (err) {
        console.error('counter readback failed', err);
        segsEl.textContent = 'err';
        debugState.segs = `err: ${err && err.message ? err.message : String(err)}`;
      } finally {
        readbackBusy = false;
      }
    }
