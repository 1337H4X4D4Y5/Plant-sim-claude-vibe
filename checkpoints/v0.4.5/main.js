...
function computeSky(t) {
  ...
  const dayFrac = 0.78;           // 78% day, 22% night
  const cyclePos = (t / dayLength) - Math.floor(t / dayLength);   // 0..1

  // Stretch the sin wave so the positive (day) half spans `dayFrac` of the
  // cycle and the negative (night) half spans `1 - dayFrac`.
  let phase;
  if (cyclePos < dayFrac) {
    phase = (cyclePos / dayFrac) * Math.PI;             // 0 → π (sunrise → sunset)
  } else {
    phase = Math.PI + ((cyclePos - dayFrac) / (1 - dayFrac)) * Math.PI;  // π → 2π (night)
  }
  const sinP = Math.sin(phase);
  const cosP = -Math.cos(phase);    // -1 at sunrise (east), +1 at sunset (west)

  // North-south wobble keyed to overall cycle, not phase, so it stays smooth
  // across the day/night seam.
...
// Per-type parameter ranges. `rand` is a 0..1 generator.
function rollByType(plantType, rand) {
  if (plantType === 1) {       // bush: wide, branchy, short
    return {
      branchAngle: 0.70 + rand() * 0.60,
      branchProb:  0.85 + rand() * 0.10,
      lenScale:    0.55 + rand() * 0.20,
      radScale:    0.50 + rand() * 0.20,
      maxDepth:    4 + Math.floor(rand() * 3),       // 4..6
      growthBias:  -0.20 + rand() * 1.00,
      seedLength:  0.30 + rand() * 0.45,
      seedRadius:  0.07 + rand() * 0.08,
    };
  }
  if (plantType === 2) {       // grass: tall, slender, no branching
    return {
      branchAngle: 0.04 + rand() * 0.12,
      branchProb:  0.00 + rand() * 0.05,
      lenScale:    0.85 + rand() * 0.10,
      radScale:    0.45 + rand() * 0.15,
      maxDepth:    4 + Math.floor(rand() * 3),
      growthBias:  0.80 + rand() * 0.80,             // mostly upright
      seedLength:  0.15 + rand() * 0.30,
      seedRadius:  0.012 + rand() * 0.022,
    };
  }
  if (plantType === 3) {       // flower: short stem with petals on top
    return {
      branchAngle: 0.30 + rand() * 0.25,
      branchProb:  0.15 + rand() * 0.30,
      lenScale:    0.55 + rand() * 0.20,
      radScale:    0.45 + rand() * 0.18,
      maxDepth:    3 + Math.floor(rand() * 2),       // 3..4
      growthBias:  0.70 + rand() * 0.90,
      seedLength:  0.18 + rand() * 0.35,
      seedRadius:  0.03 + rand() * 0.04,
    };
  }
  // Tree (default).
  return {
    branchAngle: 0.30 + rand() * 0.70,
    branchProb:  0.55 + rand() * 0.40,
    lenScale:    0.75 + rand() * 0.20,
    radScale:    0.62 + rand() * 0.25,
    maxDepth:    5 + Math.floor(rand() * 4),         // 5..8
    growthBias:  -0.20 + rand() * 1.60,
    seedLength:  0.70 + rand() * 0.80,
    seedRadius:  0.12 + rand() * 0.16,
  };
}
...
// Inherit from a parent with small random jitter per gene. plantType jumps
// rarely; when it does, the whole genome is re-rolled with the new type's
// ranges so we never get e.g. a "grass with high branchProb".
function mutateGenome(parent) {
  ...
  if (plantType !== parent.plantType) {
    // Hard type jump: re-roll the whole shape but keep the visual signature
    // (leafShape, barkHue, flowerHue) so it still feels related.
  ...
  return {
    branchAngle: clamp(jitter(parent.branchAngle, 0.15), 0.02, 1.45),
    branchProb:  clamp(jitter(parent.branchProb,  0.10), 0.00, 0.97),
    lenScale:    clamp(jitter(parent.lenScale,    0.06), 0.40, 0.97),
    radScale:    clamp(jitter(parent.radScale,    0.06), 0.30, 0.95),
    maxDepth:    clamp(parent.maxDepth + stepInt(), 3, 9),
    growthBias:  clamp(jitter(parent.growthBias,  0.30), -0.60, 1.80),
    leafShape:   Math.random() < 0.08
                   ? Math.floor(Math.random() * 5)
                   : parent.leafShape,
    barkHue:     clamp(jitter(parent.barkHue,     0.06), 0.0, 1.0),
    plantType,
    flowerHue:   clamp(jitter(parent.flowerHue,   0.08), 0.0, 1.0),
    seedLength:  clamp(jitter(parent.seedLength,  0.10), 0.05, 1.70),
    seedRadius:  clamp(jitter(parent.seedRadius,  0.02), 0.008, 0.34),
  };
}
