...
function createSimResources(device, maxPlants, segsPerPlant) {
  ...
  return {
    ...
    cpuGenomes, cpuPlantPositions, cpuPlantBirthTick, cpuPlantEnergy,
...
async function main() {
  ...
  try {
    ...
    function evolveStep(tick) {
      ...
      // 1) Energy ledger. Every plant gains energy from captured sunlight
      //    and loses energy to maintenance. Plants with no leaves capture
      //    no light and starve.
      const dead = [];
      const alive = [];
      ...
      for (let i = 0; i < CONFIG.sim.maxPlants; i++) {
        ...
        // Past grace period and out of energy → starved.
        if (age >= ENERGY_CONFIG.GRACE_TICKS && sim.cpuPlantEnergy[i] <= 0) {
          dead.push(i);
        } else if (age >= ENERGY_CONFIG.GRACE_TICKS) {
          // Eligible parent: weight by current energy (= success at staying alive).
      ...
      // 2) Replace every dead plant this tick with a mutated child of a
      //    high-energy parent. This is true natural selection — the
      //    survivors get to pass on their genes. Cap per-tick replacements
      //    so a population crash doesn't stall the frame.
      const REPLACE_CAP = 32;
      const n = Math.min(dead.length, REPLACE_CAP);
      for (let r = 0; r < n; r++) {
        const targetIdx = dead[r];
        const parentIdx = pickWeighted(scores);
        if (parentIdx === targetIdx) continue;
        replacePlant(targetIdx, parentIdx, tick);
      }
      EV.lastDeaths = dead.length;
    }
