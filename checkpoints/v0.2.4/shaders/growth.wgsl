...
// Counters layout, 16 B:
//   next         u32  next free segment slot (== current segment count)
//   dispatched   u32  thread 0 bumps this once per dispatch, proves kernel ran
//   liveTips     u32  bumped whenever a thread passes the alive+tip+age gate
//   maxIdx       u32  largest sim.tick value seen by the kernel (proves uniform OK)
struct Counters {
  next: atomic<u32>,
...
@group(0) @binding(3) var<uniform> genome: Genome;
...
fn grow(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  if (s.depth + 1.0 > genome.maxDepth) {
  ...
  // Decide branching.
  let canBranch = s.depth >= 1.0 && s.depth + 1.0 < genome.maxDepth;
  let nChildren = select(1u, 2u, canBranch && r1 < genome.branchProb);

  let base = atomicAdd(&counters.next, nChildren);
  if (base + nChildren > sim.maxSegs) {
    // Saturated: stop trying.
    segments[i].flags = s.flags & ~TIP_BIT;
    return;
  }
  ...
  let right = normalize(cross(ref, parentDir));
  ...
  for (var j: u32 = 0u; j < nChildren; j = j + 1u) {
    ...
    let tilt = select(genome.branchAngle + r3 * 0.25, (r4 - 0.5) * 0.18, isContinuation);
    let cosT = cos(tilt);
    let sinT = sin(tilt);
    let cw = cos(twist);
    let sw = sin(twist);
    ...
    // Phototropism.
    newDir = normalize(mix(newDir, vec3<f32>(0.0, 1.0, 0.0), genome.growthBias * 0.08));
