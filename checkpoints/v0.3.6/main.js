...
function createSimResources(device, maxPlants, segsPerPlant) {
  ...
  return {
    segments, counter, simBuffer, simCpu, genomeBuffer,
...
async function main() {
  ...
  try {
    const [branchCode, leafCode, skyCode, groundCode, growthCode] = await Promise.all([
      loadShader('branch'),
      loadShader('leaf'),
      loadShader('sky'),
      loadShader('ground'),
    ...
    const [branchModule, leafModule, skyModule, groundModule, growthModule] = await Promise.all([
