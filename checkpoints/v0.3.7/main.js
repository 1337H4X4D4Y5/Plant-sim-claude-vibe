...
function computeSky(t) {
  const dayLength = 90;          // seconds for a full day-night cycle
  const phase = (t / dayLength) * Math.PI * 2;
  const sinP = Math.sin(phase);
  const cosP = Math.cos(phase);
  // Sun travels in a tilted east-west arc with a slight north-south wobble
  // so it doesn't sit on the camera's right edge forever.
  const sunDir = [cosP * 0.78, sinP, Math.sin(phase * 0.62) * 0.32];

  const elevation = Math.max(0, sinP);   // 0 when below horizon
  ...
  // Cool blue ambient at night, warmer + brighter at day.
  const ambient = [
    0.05 + falloff * 0.20,
    0.07 + falloff * 0.23,
    0.11 + falloff * 0.18,
...
function createSimResources(device, maxPlants, segsPerPlant) {
  ...
  const genomeBytes = maxPlants * 32;
...
function makeGenome(seedBase, plantIdx) {
  ...
  return {
    // GPU-side fields (must match writeGenome layout):
    branchAngle: 0.30 + rand() * 0.70,        // 0.30 .. 1.00 rad (17°-57°)
    branchProb:  0.30 + rand() * 0.65,        // 0.30 .. 0.95
    lenScale:    0.65 + rand() * 0.30,        // 0.65 .. 0.95  (now actually used)
    radScale:    0.55 + rand() * 0.32,        // 0.55 .. 0.87  (now actually used)
    maxDepth:    4 + Math.floor(rand() * 5),  // 4..8 — bushes vs trees
    growthBias:  -0.40 + rand() * 1.85,       // -0.40 .. 1.45 — droopy to skyward
    leafShape:   Math.floor(rand() * 5),       // 0..4
    barkHue:     rand(),                       // 0..1, branch.wgsl tints warm→neutral→cool
    // CPU-only fields used for the initial seed segment:
    seedLength:  0.55 + rand() * 0.90,        // 0.55 .. 1.45 m trunk first segment
    seedRadius:  0.10 + rand() * 0.18,        // 0.10 .. 0.28 m trunk thickness
  };
}

function writeGenome(gf, plantIdx, g) {
  const off = plantIdx * 8;
...
// Inherit from a parent with small random jitter per gene.
function mutateGenome(parent) {
  ...
  return {
    branchAngle: clamp(jitter(parent.branchAngle, 0.15), 0.20, 1.20),
    branchProb:  clamp(jitter(parent.branchProb,  0.10), 0.20, 0.95),
    lenScale:    clamp(jitter(parent.lenScale,    0.06), 0.55, 0.95),
    radScale:    clamp(jitter(parent.radScale,    0.06), 0.45, 0.90),
    maxDepth:    clamp(parent.maxDepth + stepInt(), 3, 9),
    growthBias:  clamp(jitter(parent.growthBias,  0.30), -0.60, 1.70),
    // Leaf shape rarely jumps to keep visual lineage recognisable.
    leafShape:   Math.random() < 0.08
                   ? Math.floor(Math.random() * 5)
                   : parent.leafShape,
    barkHue:     clamp(jitter(parent.barkHue,     0.06), 0.0, 1.0),
    seedLength:  clamp(jitter(parent.seedLength,  0.12), 0.40, 1.55),
    seedRadius:  clamp(jitter(parent.seedRadius,  0.03), 0.08, 0.32),
  };
}
...
function createLeafRenderer(ctx, frame, module, segmentsBuffer, genomeBuffer, simBuffer, maxSegs) {
...
function createBranchRenderer(ctx, frame, module, segmentsBuffer, genomeBuffer, simBuffer, maxSegs) {
  const { device, format } = ctx;
...
function createLightPipelines(device, module, lightGridBuffer, segmentsBuffer, simBuffer) {
  const bgl = device.createBindGroupLayout({
    entries: [
      { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'storage' } },
  ...
  return { clearPipeline, splatPipeline, bindGroup };
}
...
async function main() {
  ...
  try {
    ...
    const light = await withScope(ctx.device, 'light-pipelines',
      () => createLightPipelines(ctx.device, lightModule, sim.lightGridBuffer, sim.segments, sim.simBuffer));
    ...
    const branches = await withScope(ctx.device, 'branch-renderer',
      () => createBranchRenderer(ctx, frame, branchModule, sim.segments, sim.genomeBuffer, sim.simBuffer, CONFIG.sim.maxSegs));
    const leaves = await withScope(ctx.device, 'leaf-renderer',
      () => createLeafRenderer(ctx, frame, leafModule, sim.segments, sim.genomeBuffer, sim.simBuffer, CONFIG.sim.maxSegs));
    ...
    const leafBuf = new Float32Array(CONFIG.sim.maxPlants * 8);
    let leafCycleIdx = 0;
    ...
    const EV = {
      ...
      perTick: 1,           // number of plants to replace each sim tick
      births: 0,
    ...
    function fitnessScore(g) {
      // Crude proxy: tall, deep, upright plants score higher. Replaced by
      // an actual light-grid fitness in v0.4.
    ...
    function evolveStep(tick) {
      ...
      // Score everyone, but only pick parents from non-retirable plants
      // (they've had time to "prove" their genome by surviving).
      const scores = new Array(CONFIG.sim.maxPlants);
      const retirable = [];
      for (let i = 0; i < CONFIG.sim.maxPlants; i++) {
        const age = tick - sim.cpuPlantBirthTick[i];
        scores[i] = age >= EV.retireAge ? 0 : fitnessScore(sim.cpuGenomes[i]);
        if (age >= EV.retireAge) retirable.push(i);
      }
      if (retirable.length === 0) return;

      for (let r = 0; r < EV.perTick; r++) {
        const targetIdx = retirable[(Math.random() * retirable.length) | 0];
    ...
    function loop(now) {
      ...
      camera.update(dt);
      const skyState = computeSky(now / 1000);
      frame.update({
        view: camera.view,
        proj: camera.proj,
      ...
      // Light grid: clear + splat every frame so the ground tracks the
      // current canopy. Auto-syncs with the render pass via the encoder.
      {
        const lpass = encoder.beginComputePass({ label: 'light' });
        lpass.setBindGroup(0, light.bindGroup);
        lpass.setPipeline(light.clearPipeline);
        lpass.dispatchWorkgroups(lightClearDispatch);
        lpass.setPipeline(light.splatPipeline);
        lpass.dispatchWorkgroups(lightSplatDispatch);
        lpass.end();
      }
