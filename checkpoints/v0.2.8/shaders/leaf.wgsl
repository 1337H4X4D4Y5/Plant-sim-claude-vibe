...
// Same world-position-keyed wind that branch.wgsl uses, so leaves and
// the branches they're attached to sway in lock-step.
...
fn vs(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VSOut {
  ...
  // Anchor position along the segment, then apply the same wind offset
  // as the branch shader. windOffset is a function of world position
  // alone, so the leaf moves with the chunk of branch it's pinned to.
