...
function createSimResources(device, maxPlants, segsPerPlant) {
  ...
  });
  {
    ...
    for (let p = 0; p < maxPlants; p++) {
      ...
      const off = slot * 12;  // 12 f32 per segment

      // Grid cell with deterministic jitter.
...
function createLeafRenderer(ctx, frame, module, segmentsBuffer, genomeBuffer, maxSegs) {
...
async function main() {
  ...
  try {
    ...
    const leaves = await withScope(ctx.device, 'leaf-renderer',
      () => createLeafRenderer(ctx, frame, leafModule, sim.segments, sim.genomeBuffer, CONFIG.sim.maxSegs));
    ...
    // Leaf-shape cycle button. Rewrites only the leafShape float (offset 24)
    // of the genome uniform so we don't disturb the rest of the buffer.
    const leafShapeBtn = document.getElementById('leaf-shape');
    const leafShapeScratch = new Float32Array(1);
    function setLeafShape(idx) {
      GENOME.leafShape = idx;
      leafShapeScratch[0] = idx;
      ctx.device.queue.writeBuffer(sim.genomeBuffer, 24, leafShapeScratch);
      leafShapeBtn.textContent = `Leaf: ${LEAF_SHAPE_NAMES[idx]} ›`;
      debugState.leafShape = LEAF_SHAPE_NAMES[idx];
    }
    setLeafShape(GENOME.leafShape);
    leafShapeBtn.addEventListener('click', () => {
      setLeafShape((GENOME.leafShape + 1) % LEAF_SHAPE_NAMES.length);
    });
